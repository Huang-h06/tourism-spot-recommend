...
def init_db():
    ...
    if count == 0:
        init_data = [
            ...
            ("成都", "成都", "5A", "休闲美食", "成都位于四川盆地西部，是古蜀文明的发祥地，拥有三千多年建城史，素有'天府之国'的美誉。这里既有宽窄巷子、锦里古街、武侯祠、杜甫草堂等深厚的历史文化底蕴，也有大熊猫繁育研究基地这样的自然瑰宝。成都还是联合国教科文组织授予的'世界美食之都'，火锅、串串、担担面、龙抄手等美食令人流连忘返。"),
            ("重庆", "重庆", "5A", "山城夜景", "重庆位于长江与嘉陵江交汇处，是中国四大直辖市之一，以'山城''雾都'著称。洪崖洞的吊脚楼夜景如梦如幻，磁器口古镇保留了老重庆的市井烟火，解放碑步行街是时尚购物的天堂。这里有世界自然遗产武隆天生三桥、大足石刻，还有穿楼而过的李子坝轻轨和长江索道，火锅文化更是享誉全国。"),
            ("青岛", "青岛", "5A", "海滨风光", "青岛位于山东半岛南部，黄海之滨，是中国著名的海滨旅游城市和历史文化名城。八大关的红瓦绿树、碧海蓝天构成了一幅欧陆风情画卷，栈桥与回澜阁是青岛的城市名片。青岛啤酒驰名中外，每年举办的国际啤酒节吸引着全球游客。崂山风景区山海相连，道教文化源远流长，是避暑纳凉的绝佳去处。")
        ]
...
# 直接运行本文件可重建本地数据库（python db_sqlite.py）
# 作为模块导入时不会自动建表，防止与 Supabase 模式冲突
